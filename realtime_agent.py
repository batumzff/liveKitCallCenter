"""
Real-time LiveKit Agent with STT-based transcript collection
"""
...
class RealTimeTranscriptCollector:
    """Collect transcript in real-time using STT events"""
    ...
    def add_user_speech(self, text: str):
        ...
        if text.strip():
            ...
            self.transcript_parts.append(f"[{timestamp}] Customer: {text}")
    ...
    def add_agent_speech(self, text: str):
        ...
        if text.strip():
            timestamp = datetime.utcnow().strftime("%H:%M:%S")
            self.transcript_parts.append(f"[{timestamp}] Agent: {text}")
    ...
    async def send_for_analysis(self):
        ...
        if self.transcript_parts:
            ...
            logger.info(f"Sent transcript for analysis: {len(self.transcript_parts)} entries")
...
@function_tool
async def schedule_callback(context: RunContext, time: str, reason: str):
...
async def entrypoint(ctx: JobContext):
    """Real-time agent with proper transcript collection"""
    logger.info(f"=== REAL-TIME AGENT STARTED ===")
    ...
    transcript_collector = RealTimeTranscriptCollector(call_id)
    ...
    try:
        ...
        # Create session with custom STT handling
        ...
        # Hook into session events for real-time transcript collection
        original_user_speech_committed = None
        original_agent_speech_committed = None
        
        # Store original handlers if they exist
        if hasattr(session, '_user_speech_committed_callbacks'):
            original_user_speech_committed = session._user_speech_committed_callbacks.copy()
        if hasattr(session, '_agent_speech_committed_callbacks'):
            original_agent_speech_committed = session._agent_speech_committed_callbacks.copy()
        ...
        def on_user_speech_committed(message):
            ...
            try:
                ...
                transcript_collector.add_user_speech(text)
                logger.info(f"Collected user speech: {text[:100]}...")
            except Exception as e:
        ...
        def on_agent_speech_committed(message):
            ...
            try:
                ...
                transcript_collector.add_agent_speech(text)
                logger.info(f"Collected agent speech: {text[:100]}...")
            except Exception as e:
        ...
        if ctx.room.name in active_calls:
            call_data = active_calls[ctx.room.name]
